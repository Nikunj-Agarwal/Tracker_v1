...
class ServoController(Node):
    ...
    def angle_callback(self, msg):
        if msg.z == 1:  # z=1 triggers incremental movement
            self.move_up()
        elif msg.z == -1:
            self.move_down()
        elif msg.x == -1:  # x=-1 for left
            self.move_left()
        elif msg.x == 1:  # x=1 for right
            self.move_right()
        else:  # Default case: move to specific angles
            target_pan = msg.x
            target_tilt = msg.y
